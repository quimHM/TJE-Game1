...
void Image::drawPun(std::string text, int x, int y, const Image& bitmapfont, int font_w, int font_h, int first_char )
{	
	int xx = x;
	int yy = y;
	for (int i = 0; i < text.size(); ++i)
	{
		yy = y + (x+i*font_w)/(width-x)*(font_h+1);
		xx = x+(i*font_w % (width-x-x));
		drawImage( bitmapfont, xx, yy, bitmapfont.getArea(text[i] - first_char, font_w, font_h));
	}
}
