...
const Color Color::BROWN(100, 50, 0);
...
Vector2 operator - (const Vector2& a, const Vector2& b) { return Vector2(a.x - b.x, a.y - b.y); }
bool operator == (const Vector2& a, const Vector2& b) { return (std::abs(a.x - b.x) + std::abs(a.y - b.y)) < 0.0001; }
...
Vector2 max_mag(Vector2 v, float l) {               
...
int planeBoxOverlap( const Vector4& plane, const Vector3& center, const Vector3& halfsize )
{
	Vector3 n = plane.xyz;
	float d = plane.w;
	float radius = std::abs(halfsize.x * n[0]) + std::abs(halfsize.y * n[1]) + std::abs(halfsize.z * n[2]);
